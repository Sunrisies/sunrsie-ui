...
import { CoordinateTransform, out_of_china } from "../../index";

describe("坐标转换工具", () => {
  describe("wgs84ToGcj02 火星坐标转换", () => {
    it("正确转换已知坐标点", () => {
      const [lng, lat] = CoordinateTransform.wgs84ToGcj02(
        114.1808934593,
        22.322230460245
      );
    ...
    it("无效参数抛出错误", () => {
      // @ts-ignore 测试错误类型
      expect(() => CoordinateTransform.wgs84ToGcj02("116", 39)).toThrow(
        "坐标必须为数字类型"
      );
      expect(() => CoordinateTransform.wgs84ToGcj02(190, 39)).toThrow(
        "坐标值应在-180到180之间"
      );
    });
  });
  ...
  describe("gcj02ToWgs84 逆向转换", () => {
    it("正确逆向转换已知点", () => {
      const [lng, lat] = CoordinateTransform.gcj02ToWgs84(
        114.1808934593,
        22.322230460245
      );
    ...
    it("多次转换应趋近原始值", () => {
      ...
      const converted = CoordinateTransform.wgs84ToGcj02(
        original[0],
        original[1]
      );
      const reverted = CoordinateTransform.gcj02ToWgs84(...converted);
